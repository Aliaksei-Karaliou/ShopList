                ...
                if (connectionCursor.moveToFirst()) {
                    ...
                    do {
                        ...
                        if (productCursor.moveToFirst()) {
                            final int nameId = productCursor.getColumnIndex(DbConstants.Product.NAME);
                            final int descriptionId = productCursor.getColumnIndex(DbConstants.Product.DESCRIPTION);
                            final int priceId = productCursor.getColumnIndex(DbConstants.Product.PRICE);
                            final int quantityId = productCursor.getColumnIndex(DbConstants.Product.QUANTITY);

                            final IProduct product = new Product.Builder(productCursor.getString(nameId))
                                    .setDescription(productCursor.getString(descriptionId))
                                    .setPrice(productCursor.getDouble(priceId))
                                    .setQuantity(productCursor.getDouble(quantityId))
                                    .build();
